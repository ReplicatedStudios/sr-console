...
const { trace } = globalThis.internalBinding('trace_events');
...
import { validateInteger } from 'internal/validators';
const { previewEntries } = internalBinding('util');
import buffer from 'buffer';
const { Buffer: { isBuffer } } = buffer;
import { inspect, formatWithOptions } from 'internal/util/inspect';
import { isTypedArray, isSet, isMap, isSetIterator, isMapIterator } from 'internal/util/types';
import { CHAR_LOWERCASE_B, CHAR_LOWERCASE_E, CHAR_LOWERCASE_N, CHAR_UPPERCASE_C } from 'internal/constants';
...
class Console {
  constructor(options /* or: stdout, stderr, ignoreErrors = true */) {
    ...
    if (!stdout || typeof stdout.write !== 'function') {
      throw 'Console expects a writable stream instance for stdout';
    }
    if (!stderr || typeof stderr.write !== 'function') {
      throw 'Console expects a writable stream instance for stderr';
    }
